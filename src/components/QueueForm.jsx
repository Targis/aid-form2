...
const QueueForm = () => {
      ...
      await fetch(hubAction, {
        ...
        .then((data) => {
          ...
          if (data.result === 'success') {
            ...
            Swal.fire(
              ...
              `<table style='text-align: left'>
                <tbody>
                  <tr><th>Номер в черзі:</th>   <td>${number}</strong></td></tr>
                  <tr><th>Дата:</th>            <td>${day}, ${date}</td></tr>
                  <tr><th>Орієнтовний час:</th> <td>${time} </td></tr>
                </tbody>
              </table>

              <br><br>
              Номер в черзі дійсний тільки в цей день. <br>
              Через повітряні тривоги, перебої з електрикою та інші обставини час може бути змінено.
              `,
              'success')
            helpers?.resetForm()
          ...
          if (data.result === 'refused') {
            Swal.fire('Відмова', 'Немає вільних місць на цю дату. Спробуйте пізніше.', 'error')
          }
  ...
  return (
    <div style={{ marginBottom: '2em' }}>
      ...
      <Typography color="red" variant="h6" component="div" sx={{ mb: 4 }}>(ТЕСТ)</Typography>

      <Typography color="inherit" variant="p" component="div" sx={{ mb: 4, textAlign: 'left' }}>
        Правила реєстрації:
        <ol>
          <li>Реєструється сім’я (домогосподарство).</li>
          <li>Першочергово реєструються особи, що евакуювалися після 01 жовтня 2022 року (вимоги гуманітарної організації "Людина в біді")</li>
          <li>Для реєстрації один член сім’ї надає необхідні дані для внесення в анкету організації (щодо себе та членів сім’ї, які з ним проживають - паспортні дані та ІНН код)</li>
          <li>Після успішної верифікації даних гуманітарною організацією, особа, що реєструвалася, отримує сповіщення на мобільний телефон щодо отримання переводу у відділенні Ощадбанку міста Запоріжжя.</li>
        </ol>
      </Typography>
